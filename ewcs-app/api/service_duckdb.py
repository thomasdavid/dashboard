...
Features:
1. Smart Filter Logic: Handles Numeric (1 vs 1.0) and String Prefix (35:54 matches 35:54:00).
2. Category Update: Uses 'ISCO_1' column.
3. Rounding: 1 decimal place.
4. Render Optimized (Memory limits).
"""
    ...
    if category_group and category_value:
        if category_group.lower() in cols_lower:
            ...
            if is_numeric_filter:
                ...
                # String Filter Logic (UPDATED: USE LIKE FOR PREFIX)
                # This ensures "35:54" matches "35:54:00" if it exists as a string
                cat_sql = f" AND CAST({category_group} AS VARCHAR) LIKE ? || '%'"
